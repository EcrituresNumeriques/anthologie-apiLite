$(document).ready(function(){
  ...
  function showEntity(data){
    ...
    for(i=0;i<data.entities.length;i++){
      ...
      for (var j = 0; j < data.entities[i].refs.length; j++) {
        $entity.children("ul.itexts").append('<li class="textText" data-id="'+data.entities[i].refs[j].id+'">'+nl2br(data.entities[i].refs[j].title)+' <i class="fa fa-arrow-circle-right goToEntity newStuff" aria-hidden="true"  data-id="'+data.entities[i].refs[j].id+'"></i></li>');
      }
      ...
      for (var j = 0; j < data.entities[i].texts.length; j++) {
        $entity.children("ul.texts").append('<li class="langText">['+data.entities[i].texts[j].translation[0].family+' / '+data.entities[i].texts[j].translation[0].lang+']</li><li class="textText" data-id="'+data.entities[i].texts[j].id+'">'+nl2br(data.entities[i].texts[j].translation[0].text)+'</li>');
      }
