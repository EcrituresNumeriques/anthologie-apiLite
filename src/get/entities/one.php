...
try{
  ...
  $getAuthors = $db->prepare("SELECT a.*,at.name,l.name as lang,l.family FROM entities_authors_assoc eaa JOIN authors a ON eaa.authors_id = a.id LEFT JOIN author_translations at ON a.id = at.author_id LEFT JOIN languages l ON at.language_id = l.id WHERE eaa.entities_id = :id");
